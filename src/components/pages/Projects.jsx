...
rounded-br-3xl xs:-right-2 sm:h-[102%] xs:w-full xs:rounded-[1.5rem]"
      ...
      > */}
        <div className="w-1/2 lg:w-full">
          <Image src={img} alt={title} className="w-full h-auto rounded-lg object-cover" />
        </div>
      {/* </Link> */}
      ...
      <div className="w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6">
        ...
        > */}
          <h2 className="my-2 w-full text-left text-4xl font-bold lg:text-2xl md:text-xl sm:text-lg">
            {title}
          </h2>
        {/* </Link> */}
...
const Project = ({ title, type, shortSummary, img, github }) => {
  return (
    <article
...
rounded-br-3xl md:-right-2 md:w-[101%] xs:h-[102%] xs:rounded-[1.5rem]"
      ...
      <div className="w-full flex flex-col items-start justify-between mt-4">
        <span className="text-primaryDark font-medium text-xl lg:text-lg md:text-base xs:text-sm">
          {type}
        </span>
        <h2 className="my-2 w-full text-left text-3xl font-bold lg:text-2xl md:text-xl sm:text-lg xs:text-base">
          {title}
        </h2>
        <p className="my-2 font-medium text-slate-50 md:text-sm xs:text-xs">{shortSummary}</p>
        <div className="w-full mt-2 flex items-center justify-between">
          ...
          <Link
            href={github}
            target="_blank"
            className="w-10 h-10 flex bg-darkGray rounded-full font-bold items-center justify-center text-center cursor-pointer border-2 border-slate-100 xs:w-8 xs:h-8"
          >
            <GithubIcon className="xs:w-4 xs:h-4" />
          </Link>
        </div>
      </div>
    </article>
    ...
    <>
      ...
      <main className="w-full mb-16 flex flex-col items-center justify-center">
        <Layout className="pt-16">
          ...
          <div className="grid grid-cols-12 gap-24 gap-y-32 text-slate-50 xl:gap-x-16 lg:gap-x-8 md:gap-y-24 sm:gap-x-0 sm:gap-y-16 xs:gap-y-12">
            ...
            <div className="col-span-6 sm:col-span-12">
              <Project
                title="SidoTanggap"
                img="/Project1.png"
                shortSummary="Platform digital yang memungkinkan masyarakat untuk melaporkan permasalahan terkait lingkungan dan sosial di sekitar mereka secara cepat dan efisien."
                github="https://github.com/vaiozaffana/sido-tanggap.git"
                type="Project"
              />
            </div>
            <div className="col-span-12">
