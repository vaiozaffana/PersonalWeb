...
const FeaturedProject = ({ type, title, summary, img, link, github }) => {
  return (
    <article className="w-full flex items-center justify-between rounded-br-2xl rounded-3xl
...
rounded-br-3xl xs:-right-2 sm:h-[102%] xs:w-full xs:rounded-[1.5rem]" />
      ...
      <div className="w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6">
        ...
        <div className="mt-2 flex items-center">
          <Link href={github} target="_blank" className="w-10 h-10 flex bg-darkGray rounded-full font-bold items-center justify-center text-center cursor-pointer border-2 border-slate-100"><GithubIcon /></Link>
          <Link href={link} target="_blank" className="ml-4 rounded-lg bg-white text-darkGray 
          p-2 px-6 text-lg font-semibold sm:px-4 sm:text-base">Visit Project</Link>
        </div>
      </div>
    </article>
  );
...
<div className="absolute top-0 -right-3 -z-10 w-[101%] h-[103%] rounded-[2rem] bg-white
rounded-br-3xl md:-right-2 md:w-[101%] xs:h-[102%] xs:rounded-[1.5rem]" />
      ...
      <div className="w-full flex flex-col items-start justify-between mt-4">
        <span className="text-primaryDark font-medium text-xl lg:text-lg md:text-base">{type}</span>
        <Link href={link} target="_blank" className="hover:underline underline-offset-2">
          <h2 className="my-2 w-full text-left text-3xl font-bold lg:text-2xl">{title}</h2>
        </Link>
        <div className="w-full mt-2 flex items-center justify-between">
        <Link href={link} target="_blank" className="text-lg font-semibold hover:underline md:text-base">Visit</Link>
          <Link href={github} target="_blank" className="w-10 h-10 flex bg-darkGray rounded-full font-bold items-center justify-center text-center cursor-pointer border-2 border-slate-100"><GithubIcon /></Link>
        </div>
      </div>
    </article>
